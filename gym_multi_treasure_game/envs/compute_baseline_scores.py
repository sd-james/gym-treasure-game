...
from gym_multi_treasure_game.envs.eval2 import evaluate_other, sample_paths, evaluate_one_step, evaluate_n_step, \
    compute_histogram
from gym_multi_treasure_game.envs.evaluate import evaluate_similarity_pretrained, draw
...
def compute(tasks, N, exact_length, n_exps=5):
    ...
    for experiment in range(n_exps):
        for task in tasks:
            ...
            # paths = sample_paths(ground_truth, 10)
...
if __name__ == '__main__':
    ...
    if not exists('nsamples.pkl'):
        save(get_n_samples(), 'nsamples.pkl')

    import warnings

    warnings.filterwarnings("ignore")

    n_samples = load('nsamples.pkl')
    ...
    functions = [partial(compute, split[i], N, exact_length, n_exps=n_exps) for i in range(len(split))]
    ...
    save(merged, 'baseline.pkl')
    save(merged_stats, 'baseline_stats.pkl')
